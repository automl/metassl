...
def find_experiments(expt_dir, search_keys):
    ...
    for subdir, dirs, files in os.walk(expt_dir):
        ...
        if (
            os.path.exists(subdir / "summary_dict.npy")
            and os.path.exists(subdir / "config.yml")
            and all([key in subdir.__str__() for key in search_keys])
        ):
            config = ConfigHandler(config_file=subdir / "config.yml")
            # expt_name = config.expt.session_name + "/" + \
            #             config.expt.experiment_name.split("_seed")[0]
            ...
            expt_dict = {
                ...
                "expt_name": expt_name,
                "seed": seed,
                "expt_folder": config.expt.experiment_name,
            }
