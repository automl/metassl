...
try:
    ...
    from .data import normalize_cifar10, normalize_cifar100, normalize_imagenet  # noqa F401
...
class DataAugmentation(nn.Module):
    ...
    def __init__(self, config) -> None:
        ...
        self.aug_w_b = nn.Parameter(
            torch.zeros(len(self.color_jitter_strengths_brightness)), requires_grad=True
        )
        self.aug_w_c = nn.Parameter(
            torch.zeros(len(self.color_jitter_strengths_contrast)), requires_grad=True
        )
        self.aug_w_s = nn.Parameter(
            torch.zeros(len(self.color_jitter_strengths_saturation)), requires_grad=True
        )
        self.aug_w_h = nn.Parameter(
            torch.zeros(len(self.color_jitter_strengths_hue)), requires_grad=True
        )

        bound = 1.0 / math.sqrt(self.aug_w_b.size(0))
        bound_h = 1.0 / math.sqrt(self.aug_w_h.size(0))
        ...
        # histograms
        self.color_jitter_histogram_brightness = {
            k: 0 for k in self.color_jitter_strengths_brightness
        }
        self.color_jitter_histogram_contrast = {k: 0 for k in self.color_jitter_strengths_contrast}
        self.color_jitter_histogram_saturation = {
            k: 0 for k in self.color_jitter_strengths_saturation
        }
    ...
    def sample_logprobs(self):
        color_jitter_action_idx_b, color_jitter_logprob_b, _ = get_sample_logprob(
            logits=self.aug_w_b
        )
        color_jitter_action_idx_c, color_jitter_logprob_c, _ = get_sample_logprob(
            logits=self.aug_w_c
        )
        color_jitter_action_idx_s, color_jitter_logprob_s, _ = get_sample_logprob(
            logits=self.aug_w_s
        )
        color_jitter_action_idx_h, color_jitter_logprob_h, _ = get_sample_logprob(
            logits=self.aug_w_h
        )
        ...
        if self.expt_mode == "ImageNet":
            modules.append(
                RandomApply(
                    [
                        ColorJitter(
                            brightness=strength_b,
                            contrast=strength_c,
                            saturation=strength_s,
                            hue=strength_h,
                        )
                    ],
                    p=0.8,
            ...
            modules.append(RandomApply([GaussianBlur([kernel_h, kernel_w], [0.1, 2.0])], p=0.5))
            modules.append(RandomHorizontalFlip())
        elif self.expt_mode == "CIFAR10":
            modules.append(RandomResizedCrop(size=32, scale=(0.2, 1.0)))
            modules.append(
                RandomApply(
                    [
                        ColorJitter(
            ...
            raise ValueError(
                "For now, only ImageNet and CIFAR10 are supported when parameterizing "
                "augmentations with nn module"
            )
...
def create_transforms(
    ...
    if isinstance(strengths_b, float):
        strengths_b, strengths_c, strengths_s, strengths_h = (
            [strengths_b],
            [strengths_c],
            [strengths_s],
            [strengths_h],
    ...
    assert (
        len(strengths_b) == len(strengths_c) == len(strengths_s) == len(strengths_h)
    ), "strengths must all have the same length"
        ...
        trans = TwoCropsTransform(
            transforms.Compose(
                [
                    transforms.RandomApply(
                        [
                            transforms.ColorJitter(
                                brightness=strengths_b[i],
                                contrast=strengths_c[i],
                                saturation=strengths_s[i],
                                hue=strengths_h[i],
                            )
                        ],
                        p=0.8,
                    ...
                    transforms.RandomApply(
                        [torchvision.transforms.GaussianBlur([kernel_h, kernel_w], [0.1, 2.0])],
                        p=0.5,
                    ),
