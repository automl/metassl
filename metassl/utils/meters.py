...
class ExponentialMovingAverageMeter(AverageMeter):
    ...
    def update(self, val, n=1):
        super().update(val, n)
        self.ema = (self.val * (self.alpha / (1 + self.window))) + self.ema * (
            1 - (self.alpha / (1 + self.window))
        )
...
def update_grad_stats_meters(
    grads, meters, warmup, parameterize_augmentations=False, aug_model=None
):
    backbone_grads_pt_lw, backbone_grads_pt_global = (
        grads["backbone_grads_pt_lw"],
        grads["backbone_grads_pt_global"],
    )
    backbone_grads_ft_lw, backbone_grads_ft_global = (
        grads["backbone_grads_ft_lw"],
        grads["backbone_grads_ft_global"],
    )
    ...
    if not warmup:
        if backbone_grads_ft_lw is not None:
            mean, std = calc_layer_wise_stats(
                backbone_grads_pt=backbone_grads_pt_lw,
                backbone_grads_ft=backbone_grads_ft_lw,
                metric_type="cosine",
            )
            ...
            mean, std = calc_layer_wise_stats(
                ...
                backbone_grads_ft=backbone_grads_ft_lw,
                metric_type="cosine",
                standardize_cosine=True,
            )
            cos_sim_ema_meter_standardized_lw.update(
                mean
            ), cos_sim_std_meter_standardized_lw.update(std)
        ...
        if backbone_grads_ft_global is not None:
            cos_sim_ema_meter_global.update(
                F.cosine_similarity(backbone_grads_pt_global, backbone_grads_ft_global, dim=0)
            )
            backbone_grads_pt_global_standardized = (
                backbone_grads_pt_global - backbone_grads_pt_global.mean()
            ) / backbone_grads_pt_global.std()
            backbone_grads_ft_global_standardized = (
                backbone_grads_ft_global - backbone_grads_ft_global.mean()
            ) / backbone_grads_ft_global.std()
            cos_sim_ema_meter_standardized_global.update(
                F.cosine_similarity(
                    backbone_grads_pt_global_standardized,
                    backbone_grads_ft_global_standardized,
                    dim=0,
            ...
            dot_prod_meter_global.update(
                torch.dot(backbone_grads_pt_global, backbone_grads_ft_global)
            )
            eucl_dis_meter_global.update(
                torch.linalg.norm(backbone_grads_pt_global - backbone_grads_ft_global, 2)
            )
        ...
        # global
        cos_sim_ema_meter_global.update(0.0)
        cos_sim_ema_meter_standardized_global.update(0.0)
        dot_prod_meter_global.update(0.0)
        eucl_dis_meter_global.update(0.0)
        norm_pt_meter_global.update(torch.linalg.norm(backbone_grads_pt_global, 2))
        norm_ft_meter_global.update(0.0)

        # layer-wise
        cos_sim_ema_meter_lw.update(0.0), cos_sim_std_meter_lw.update(0.0)
        cos_sim_ema_meter_standardized_lw.update(0.0), cos_sim_std_meter_standardized_lw.update(0.0)

        dot_prod_avg_meter_lw.update(0.0), dot_prod_std_meter_lw.update(0.0)
        eucl_dis_avg_meter_lw.update(0.0), eucl_dis_std_meter_lw.update(0.0)
        ...
        norm_ft_avg_meter_lw.update(0.0), norm_ft_std_meter_lw.update(0.0)

        if parameterize_augmentations:
            reward_meter.update(0.0)
            norm_aug_brightness_grad_meter.update(0.0)
            norm_aug_contrast_grad_meter.update(0.0)
            norm_aug_saturation_grad_meter.update(0.0)
            norm_aug_hue_grad_meter.update(0.0)
...
def calc_all_layer_wise_stats(
    ...
    if not warmup:
        ...
        cos_sim_ema_meter.update(0.0), cos_sim_std_meter.update(0.0)
        dot_prod_avg_meter.update(0.0), dot_prod_std_meter.update(0.0)
        eucl_dis_avg_meter.update(0.0), eucl_dis_std_meter.update(0.0)
        ...
        norm_ft_avg_meter.update(0.0), norm_ft_std_meter.update(0.0)


def calc_layer_wise_stats(
    backbone_grads_pt,
    backbone_grads_ft=None,
    metric_type="cosine",
    standardize_cosine=False,
):
    ...
    if backbone_grads_ft is not None:
        for (k1, v1), (k2, v2) in zip(backbone_grads_pt.items(), backbone_grads_ft.items()):
            # todo: for each layer, assumes independence of weights and biases but should be
            #  considered as one layer
...
def initialize_all_meters():
    ...
    # global meters
    cos_sim_ema_meter_global = ExponentialMovingAverageMeter(
        "Cos. Sim. PT-FT global. average", window=100, alpha=2, fmt=":6.4f"
    )
    cos_sim_ema_meter_standardized_global = ExponentialMovingAverageMeter(
        "Cos. Sim. (standardized) PT-FT global. average",
        window=100,
        alpha=2,
        fmt=":6.4f",
    )
    ...
    # layer-wise meters
    cos_sim_ema_meter_lw = ExponentialMovingAverageMeter(
        "Cos. Sim. PT-FT layer-w. average", window=100, alpha=2, fmt=":6.4f"
    )
    cos_sim_std_meter_lw = AverageMeter("Cos. Sim. PT-FT layer-w. std.", ":6.4f")

    cos_sim_ema_meter_standardized_lw = ExponentialMovingAverageMeter(
        "Cos. Sim. (standardized) PT-FT layer-w. average",
        window=100,
        alpha=2,
        fmt=":6.4f",
    )
    cos_sim_std_meter_standardized_lw = AverageMeter(
        "Cos. Sim. (standardized) PT-FT layer-w. std.", ":6.4f"
    )
