try:
    ...
    from metassl.utils.meters import ExponentialMovingAverageMeter
except ImportError:
    # For execution in command line
    from .meters import ExponentialMovingAverageMeter
...
def write_to_summary_writer(
    ...
    if optimizer_pt:
        writer.add_scalar(
            "Learning rate/pre-training", optimizer_pt.param_groups[0]["lr"], total_iter
        )
        if "weight_decay" in optimizer_pt.param_groups[0]:
            writer.add_scalar(
                "Weight Decay/pre-training",
                optimizer_pt.param_groups[0]["weight_decay"],
                total_iter,
            )
    if optimizer_ft:
        writer.add_scalar(
            "Learning rate/fine-tuning", optimizer_ft.param_groups[0]["lr"], total_iter
        )
        if "weight_decay" in optimizer_ft.param_groups[0]:
            writer.add_scalar(
                "Weight Decay/fine-tuning",
                optimizer_ft.param_groups[0]["weight_decay"],
                total_iter,
            )
    ...
    for stat in main_stats_meters:
        if isinstance(stat, ExponentialMovingAverageMeter):
            ...
            writer.add_scalar(
                f"Advanced Stats/{stat.name} exp. moving average", stat.ema, total_iter
            )
        else:
            writer.add_scalar(f"Advanced Stats/{stat.name}", stat.val, total_iter)
            writer.add_scalar(f"Advanced Stats/{stat.name} average", stat.avg, total_iter)
    ...
    for stat in additional_stats_meters:
        if isinstance(stat, ExponentialMovingAverageMeter):
            ...
            writer.add_scalar(
                f"Additional Advanced Stats/{stat.name} average", stat.avg, total_iter
            )
            # exponential moving average
            writer.add_scalar(
                f"Additional Advanced Stats/{stat.name} exp. moving average",
                stat.ema,
                total_iter,
            )
        else:
